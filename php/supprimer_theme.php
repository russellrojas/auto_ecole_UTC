<html>
    ...
    <?php
        ...
        $idtheme_ech = pg_escape_string($connect,$idtheme);
        ...
        $query_supr_theme = "UPDATE themes SET supprime = true WHERE idtheme=$idtheme_ech";
        $result_supr_theme = pg_query($connect, $query_supr_theme);
        if (!$result_supr_theme) {
            echo "La requete $query_supr_theme a échoué : ".pg_last_error($connect);
            echo "<br><a href=\"suppression_theme.php\" >Recommencer en cliquant ici</a>";
            exit();
        }
        ...
        $result_futur_seances = pg_query($connect, $query_futur_seances);

        if (!$result_futur_seances) {
            echo "La requete $query_futur_seances a échoué : ".pg_last_error($connect);
            echo "<br><a href=\"suppression_theme.php\" >Recommencer en cliquant ici</a>";
            exit();
        }
        ...
        if (pg_num_rows($result_futur_seances)>0) {
            $nb_seances_restants=pg_num_rows($result_futur_seances);
