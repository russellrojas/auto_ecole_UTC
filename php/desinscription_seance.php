    ...
    <form action="desinscrire_seance.php" method="post">
        <table>
          ...
          <?php
            ...
            $result_eleves = pg_query($connect, $query_eleves);

            if (!$result_eleves) {
              echo "La requete $query_eleves a échoué : ".pg_last_error($connect);
            ...
            $result_seances = pg_query($connect, $query_seances);

            if (!$result_seances) {
              echo "La requete $query_seances a échoué : ".pg_last_error($connect);
              echo "<br><a href=\"desinscription_seance.php\" >Recommencer en cliquant ici</a>";
              exit();
            }
            ...
            while ($row = pg_fetch_array($result_eleves))
            {
            ...
            while ($row2 = pg_fetch_array($result_seances))
            {
