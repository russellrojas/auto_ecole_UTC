<html>
    ...
    <?php
      ...
      $idseance_ech = pg_escape_string($connect, $idseance);
      ...
      $result_inscrits = pg_query($connect, $query);
      ...
      while ($ligne_eleves = pg_fetch_assoc($result_inscrits)){ // les eleves inscrits
        ...
        if (is_numeric($nb_fautes) and $nb_fautes<=40 and $nb_fautes>=0) {// verif nbombre de fautes est nombre compris entre  0 et 40
            ...
            $update_note = pg_query($connect, "UPDATE inscription set note = $note where ideleve = $ideleve and idseance = $idseance;");
            if (!$update_note) {
              echo "échoué :" .pg_last_error($connect);
