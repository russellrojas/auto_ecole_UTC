<html>
    ...
    <body >
      <?php  
        ...
        $result_inscription = pg_query($connect, $query_inscription);

        if (!$result_inscription) {
            echo "La requete $query_inscription a échoué : ".pg_last_error($connect);
            echo "<br><a href=\"desinscription_seance.php\" >Recommencer en cliquant ici</a>";
            exit();
        }

        if (pg_num_rows($result_inscription) == 0) { // pverifier si l'éleve etait inscrit dans cette seance
